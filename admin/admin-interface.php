...
function optionsframework_add_admin() {
    ...
    $of_page = add_menu_page($themename, "Tema Ayarları", "edit_theme_options", "optionsframework","optionsframework_options_page", get_bloginfo("template_url")."/admin/images/panel.png"); // Default
...
function optionsframework_options_page(){
    ...
    $themename =  get_option('of_themename');
?>
<div style="margin:20px 0 0 190px;"> <a href="https://hptnc.com" target="_blank">HPTNC</a> 'dizin teması' 25 Mart 2020 tarihinde güncellendi.</div>
...
<div class="wrap" id="of_container">
  ...
  <form action="<?php echo esc_attr( $_SERVER['REQUEST_URI'] ) ?>" method="post" style="display:inline;" id="ofform-reset">
    ...
    <input name="reset" type="submit" value="Ayarları Sıfırla" class="button submit-button reset-button" onclick="return confirm('Dikkat: Yaptığınız değişiklikler silinecektir');" />
...
function optionsframework_machine($options) {
	...
	foreach ($options as $value) {
		...
		case "typography":
			...
			if (strpos($val, 'Poppins, sans-serif') !== false){ $font01 = 'selected="selected"'; }
			if (strpos($val, 'Poppins, Geneva') !== false){ $font02 = 'selected="selected"'; }
			if (strpos($val, 'Poppins') !== false){ $font03 = 'selected="selected"'; }
			if (strpos($val, 'Poppins') !== false){ $font04 = 'selected="selected"'; }
			if (strpos($val, 'Poppins') !== false){ $font05 = 'selected="selected"'; }
			if (strpos($val, 'Poppins, Geneva') !== false){ $font06 = 'selected="selected"'; }
			if (strpos($val, 'Poppins') !== false){ $font07 = 'selected="selected"'; }
			if (strpos($val, 'Poppins') !== false){ $font08 = 'selected="selected"'; }
			
			$output .= '<select class="of-typography of-typography-face" name="'. $value['id'].'_face" id="'. $value['id'].'_face">';
			$output .= '<option value="Poppins, sans-serif" '. $font01 .'>Poppins</option>';
			$output .= '<option value="Poppins, Geneva, sans-serif" '. $font02 .'>Poppins</option>';
			$output .= '<option value="&quot;Poppins&quot;, Tahoma, sans-serif"'. $font03 .'>Poppins</option>';
			$output .= '<option value="Poppins, serif" '. $font04 .'>Georgia</option>';
			$output .= '<option value="&quot;Poppins&quot;, serif"'. $font05 .'>Poppins</option>';
			$output .= '<option value="Poppins, Geneva, Verdana, sans-serif"'. $font06 .'>Poppins</option>';
			$output .= '<option value="Poppins, &quot;Palatino Linotype&quot;, serif"'. $font07 .'>Poppins</option>';
			$output .= '<option value="&quot;Poppins&quot;, Helvetica, sans-serif" '. $font08 .'>Poppins*</option>';
			$output .= '</select>';
